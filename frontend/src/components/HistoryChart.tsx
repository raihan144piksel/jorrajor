...
const HistoryChart: React.FC<HistoryChartProps> = ({ data }) => {
  ...
  return (
    <div className="w-full min-w-0">
      ...
      {data.length === 1 && (
        <div className="mt-4 p-3 bg-slate-900/50 rounded-xl border border-slate-700/30 text-center">
          <p className="text-xs text-slate-400 font-medium">
            ℹ️ Hanya 1 titik data yang tersedia pada rentang waktu ini karena
            perangkat tidak aktif atau data tidak cukup.
          </p>
        </div>
      )}
    </div>
